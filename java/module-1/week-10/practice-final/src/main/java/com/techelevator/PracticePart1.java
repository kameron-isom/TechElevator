...
public class PracticePart1 {
	...
	*/
	public double Q01_removeFraction(double numerator, double denominator) {
		double result = (int)(numerator / denominator);
		return result;
	}
	...
	*/
	public int Q04_carWashPrice(char typeOfWash, boolean isMorning, boolean isWeekend) {
		int total = 8;	// Assume basic price
		if (typeOfWash == 'P') {
			total = 10;
	...
	*/
	public String Q06_stringBetweenTags(String stringToSearch, String tag) {
		String result = "";
		int startPosition = stringToSearch.indexOf(tag);
		int stopPosition = stringToSearch.indexOf(tag, startPosition + 1);
		if (startPosition != -1 && stopPosition > startPosition) {
			result = stringToSearch.substring(startPosition + tag.length(), stopPosition);
		}
		return result;
	}
...
//		}
		// Alternative 1 fix -- prepend the char to the reversedWord
//		for (int i = 0; i < word.length(); i++) {
//			reverseWord = word.charAt(i) + reverseWord;
//		}
		// Alternative 2 fix -- loop from end to beginning of word
...
//		 // Alternative 1 solution -- in-place swap
	...
	*/
	public Map<String, Integer> Q11_idealPets(String[] suggestedPets) {
		...
		for (String pet : suggestedPets) {
			if (idealPets.containsKey(pet)) {
				idealPets.replace(pet, idealPets.get(pet) + 1);
			} else {
				idealPets.put(pet, 1);
			}
		}
